...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, onLoginSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
        <div className="w-full max-w-sm bg-[#1e293b] border border-slate-700 rounded-2xl shadow-2xl z-10 p-8 text-white animate-in fade-in zoom-in-95" onClick={e => e.stopPropagation()}>
            ...
            <div className="text-center mb-8">
                ...
                <h1 className="text-2xl font-bold text-white mb-1">Start Building.</h1>
                <p className="text-slate-400 text-sm">Create a free account to begin.</p>
            </div>
            ...
            <div className="space-y-4">
                {showEmailForm ? (
                    <form onSubmit={handleEmailSubmit} className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-[#0f172a] border border-gray-700 rounded-lg py-2.5 pl-10 pr-4 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                        ...
                        {error && (<div className="p-3 bg-red-900/30 border border-red-800 rounded-lg text-red-200 text-sm">{error}</div>)}
                        ...
                        <button 
                            ...
                            className="w-full text-center text-sm text-slate-400 hover:text-white pt-2 transition-colors"
                        >
                    ...
                    <>
                        <button onClick={() => handleSocialLogin('google')} disabled={!!loading} className="w-full justify-center text-sm font-semibold py-3 px-4 rounded-lg border border-slate-700 bg-slate-800/50 hover:bg-slate-700 transition-colors flex items-center gap-3 disabled:opacity-50">
                        ...
                        <button onClick={() => handleSocialLogin('github')} disabled={!!loading} className="w-full justify-center text-sm font-semibold py-3 px-4 rounded-lg border border-slate-700 bg-slate-800/50 hover:bg-slate-700 transition-colors flex items-center gap-3 disabled:opacity-50">
                        ...
                        <button onClick={() => setShowEmailForm(true)} className="w-full text-sm font-semibold py-3 px-4 rounded-lg bg-white text-slate-900 hover:bg-slate-200 transition-colors">Continue with email</button>
                        <p className="text-xs text-slate-500 text-center px-4">By continuing, you agree to the <a href="#" className="underline hover:text-slate-300">Terms of Service</a> and <a href="#" className="underline hover:text-slate-300">Privacy Policy</a>.</p>
                    </>
                )}
            </div>
        </div>
