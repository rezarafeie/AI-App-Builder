...
export const rafieiCloudService = {
    ...
    async getLogs(projectRef: string) {
        try {
            return await requestManagement(`/projects/${projectRef}/analytics/endpoints/logs.all`, 'GET');
        } catch(e) { return []; }
    },
    ...
    // --- DATA OPERATIONS (Admin via SQL or Project API) ---
    ...
    async getAuthUsers(projectRef: string) {
        const sql = `SELECT id, email, created_at, role, last_sign_in_at FROM auth.users`;
        const res = await this.executeSql(projectRef, sql);
