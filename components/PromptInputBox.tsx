...
const PromptInputBox: React.FC<PromptInputBoxProps> = ({ onSendMessage, isThinking, onInteraction }) => {
  ...
  return (
    ...
    >
        {isDragging && (
            <div className="absolute inset-0 bg-slate-800/80 backdrop-blur-sm z-30 flex flex-col items-center justify-center pointer-events-none border-2 border-dashed border-indigo-400 rounded-3xl">
                <ImageIcon size={40} className="text-indigo-300 mb-2" />
                <p className="font-semibold text-white">Drop images to attach</p>
            </div>
        )}
        <form onSubmit={handleSubmit} className="relative group bg-slate-900/70 backdrop-blur-xl rounded-2xl border border-slate-700/50 shadow-2xl shadow-black/30">
          {stagedImages.length > 0 && (
            <div className="p-3 border-b border-slate-700/50">
                <div className="flex gap-3 overflow-x-auto">
                    {stagedImages.map((img) => (
                        <div key={img.id} className="relative shrink-0 w-16 h-16 rounded-lg overflow-hidden border border-slate-600 group/img">
          ...
          <div className="flex p-2 items-start">
            <button type="button" onClick={() => fileInputRef.current?.click()} className="p-3 text-slate-400 hover:text-indigo-300 transition-colors shrink-0"><Plus size={20} /></button>
            ...
            <textarea
                ...
                placeholder="Hi" 
                disabled={isThinking} 
                className="w-full bg-transparent text-white placeholder-slate-500 focus:outline-none py-3 resize-none max-h-40 leading-relaxed" 
            />
            <div className="p-1 self-end">
                <button 
                    type="submit" 
                    disabled={(!input.trim() && stagedImages.length === 0) || isThinking}
                    className="p-2.5 bg-white rounded-full text-slate-900 hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
