        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onClose}>
            <div className="w-full max-w-3xl bg-[#1e293b] border border-slate-700 rounded-2xl shadow-2xl p-8 animate-in fade-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h2 className="text-xl font-bold text-white flex items-center gap-2"><Globe size={20}/> Manage Domains</h2>
                        <p className="text-slate-400 text-sm">Add or manage custom domains for your project.</p>
                    </div>
                ...
                <div className="bg-slate-900/50 border border-slate-700 rounded-lg max-h-[50vh] overflow-y-auto">
                    {isLoading ? <div className="p-8 text-center text-slate-500">Loading domains...</div> : 
                        ...
                        <table className="w-full text-sm">
                            <thead className="sticky top-0 bg-slate-900/80 backdrop-blur-sm">
                                <tr>
                                    ...
                                    <th className="text-left font-semibold p-3">Configuration</th>
                            ...
                            <tbody>
                                {domains.map(d => (
                                    <tr key={d.id} className="border-t border-slate-800">
                                        <td className="p-3 font-medium">{d.domainName} {d.isPrimary && <span className="text-xs bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded-full ml-2">Primary</span>}</td>
                                        <td className="p-3">
                                            <div className="grid grid-cols-[50px_1fr] items-center gap-x-2 gap-y-1">
                                                <span className="text-slate-500 text-right">Type</span> <span className="bg-slate-800 text-slate-300 px-2 py-0.5 rounded text-xs font-mono w-fit">{d.dnsRecordType}</span>
                                                <span className="text-slate-500 text-right">Value</span> <CodeBlock value={d.dnsRecordValue || ''} />
                                            </div>
                                        </td>
                                        <td className="p-3">
                                            {d.status === 'verified' && <div className="flex items-center gap-1.5 text-green-400"><CheckCircle2 size={14}/> Verified</div>}
                                            {d.status === 'pending' && <button onClick={() => handleVerifyDomain(d.id)} className="flex items-center gap-1.5 text-yellow-400 hover:text-yellow-300"><Loader2 size={14} className="animate-spin"/> Verify</button>}
                                            {d.status === 'error' && <div className="flex items-center gap-1.5 text-red-400"><AlertCircle size={14}/> Error</div>}
                                        </td>
                ...
                <div className="mt-6 pt-6 border-t border-slate-800">
                    <h3 className="font-semibold mb-2">Add a new domain</h3>
                    <form onSubmit={handleAddDomain} className="flex gap-2">
                        <input type="text" value={newDomain} onChange={(e) => setNewDomain(e.target.value)} placeholder="e.g., my-awesome-app.com" className="flex-1 bg-slate-800/50 border border-slate-700 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500" />
                    ...
                    <p className="text-xs text-slate-500 mt-2">Both root domains (example.com) and subdomains (app.example.com) are supported.</p>
                </div>
            </div>
        </div>
    );
